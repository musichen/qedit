...
- **App icons**: `src-tauri/icons/qedit_logo.svg` is the canonical vector source. Regenerate the full platform set with `pnpm tauri icon src-tauri/icons/qedit_logo.svg -o src-tauri/icons`; `src-tauri/tauri.conf.json` intentionally bundles the generated `icons/icon.png`.

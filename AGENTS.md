...
- **App icons**: `src-tauri/icons/qedit_logo_centered.png` is the canonical raster artwork and `qedit_logo.svg` is the vector source. Regenerate the full platform set with `pnpm tauri icon src-tauri/icons/qedit_logo_centered.png -o src-tauri/icons`; `src-tauri/tauri.conf.json` intentionally bundles the generated `icons/icon.png`.

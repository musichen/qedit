...
- **macOS packaging**: `scripts/build-mac.sh` is the DMG entrypoint; it removes Tauri's `rw.*.dmg` temporary images, requests both `app,dmg` targets, and asserts both final bundles so repeated or release builds do not depend on stale staging output.

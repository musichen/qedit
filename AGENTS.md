...
- **Terminal**: the integrated terminal is a Rust-owned PTY (`portable-pty`) in `src-tauri/src/lib.rs` exposed as the `terminal_spawn`/`terminal_write`/`terminal_resize`/`terminal_close` commands, driven by an Xterm.js front end in `src/components/TerminalPanel.tsx`. Shell startup is configured in `lib.rs`: `$SHELL` wins, then the platform account shell, with startup flags chosen per shell family (`shell_arguments`/`shell_family`) — POSIX shells get `-il`, csh/tcsh get `-l` alone, pwsh and unrecognised shells get no flags — plus an explicit `TERM`/essential environment. Invariants, all covered by the `#[cfg(test)]` tests in `lib.rs` and `scripts/native-smoke.sh`: every session cwd goes through `safe_home_path` (canonicalized and rejected outside `$HOME`), a closed session is deregistered so later writes fail, and `RunEvent::Exit` kills every live shell so no orphan processes survive.

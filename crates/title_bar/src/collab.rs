...
use crate::{TitleBar, debug_title_bar};
...
impl TitleBar {
    ...
    pub(crate) fn render_call_controls(
        ...
        let Some(room) = ActiveCall::global(cx).read(cx).room().cloned() else {
            debug_title_bar("render_call_controls no active room");
            return Vec::new();
        };
        ...
        debug_title_bar(format!(
            "render_call_controls room_id={} channel_id={:?} child_count={} can_use_microphone={} can_share_projects={} screen_sharing_supported={}",
            room_id,
            channel_id,
            children.len(),
            can_use_microphone,
